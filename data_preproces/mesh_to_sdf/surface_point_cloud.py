...
from .utils import sample_uniform_points_in_unit_sphere, sample_uniform_points_in_voxels
...
class SurfacePointCloud:
    ...
    def sample_sdf_near_surface(self, voxels=None, voxel_size=1, number_of_points=500000, use_scans=True, sign_method='normal', normal_sample_count=11, min_size=0, return_gradients=False):
        query_points = []
        ...
        query_points.append(surface_points + np.random.normal(scale=0.005, size=(surface_sample_count, 3)))
        query_points.append(surface_points + np.random.normal(scale=0.0005, size=(surface_sample_count, 3)))
        
        if voxels is not None:
            unit_sphere_points = sample_uniform_points_in_voxels(voxels, 32, voxel_size)
        else:
